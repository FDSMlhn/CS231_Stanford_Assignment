...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    # shape (N, C).                                                             #
    #############################################################################
    H1 = np.dot(X, W1) + b1.reshape(1,-1) #shape(N, H1)
    ...
    temp = H2 - np.max(H2, axis=1,keepdims= True)
    #print(temp[np.arange(N),y])
    
    temp = np.exp(temp)
    # print(temp[np.arange(N),y])
    # print('kkkkkkkk')
    true_prob = temp[np.arange(N),y]/ np.sum(temp,1)
    #print(true_prob)
    #true_prob[true_prob==0] = 1e-37
  ...
  def train(self, X, y, X_val, y_val,
    ...
    for it in range(num_iters):
      ...
      for param in self.params:
          self.params[param] -= learning_rate*learning_rate_decay* grads[param]
